...
def main(num_subj, set_seed, method) :
    X, W, proj = get_data(num_subj, set_seed, method)
    ...
    print "isi : %f \t num_subj %d \t set_seed %d \t method %s"%(isi, num_subj, set_seed, method)
    save_stuff(num_subj, set_seed, W, proj, isi, cost, method)
...
def get_data(num_subj, set_seed, method) :
    data = loadmat("SCV_IVA_rand_proj_W_subj%d_start%d_method_%s.mat"%(num_subj, set_seed, method))
    W = data['W']
    X = data['X']
    proj = data['proj']
    return X, W, proj

def save_stuff(num_subj, set_seed, W, proj, isi, cost, method) :
    fil = open("test_noIVAG_pooled_subj%s_start%s_method%s.txt"%(index4(num_subj), index2(set_seed), method), 'w')
    fil.write(str(isi) + ", " + str(num_subj) + ", " + str(set_seed) + "\n")
    for i in cost :
        fil.write(str(i) + "\n")
    fil.close()
    
    savemat("test_IVAG_pooled_subj%s_start%s_method_%s.mat"%(index4(num_subj), index2(set_seed), method), {'W':W})
